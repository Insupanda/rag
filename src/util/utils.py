...
def insurance_keywords_mapping() -> dict[str, list[str]]:
    insu_filename = [insu_match.__dict__.values()]
...
def find_matching_collections(question, available_collections) -> list[str]:
    """
    ...
    detected_comparison = any(keyword in normalized_question for keyword in comparison_keywords)
    ...
    if len(mentioned_companies) > 1 or detected_comparison or "암" in detected_insurance_types:
        # 모든 보험사 컬렉션 추가
        matched_collections.extend(list(insu_match.__dict__.values()))

    else:
        print("mentioned", mentioned_companies)
        if mentioned_companies in list(insu_match.__dict__.keys()):
            mentioned_index = list(insu_match.__dict__.keys()).index(mentioned_companies[0])
            matched_collections.append(list(insu_match.__dict__.values())[mentioned_index])
